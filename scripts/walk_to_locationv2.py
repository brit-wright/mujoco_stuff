...
from QuadrupedControllerv2 import QuadrupedController
    ...
    while (not glfw.window_should_close(window)) and (t_sim < max_sim_time):
        ...
        while (t_sim - t0_sim) < (dt_render):
            ...
            if counter % decimation == 0:
                ...
                distance_error = sqrt((goal_pos[0] - p_curr[0])**2 + (goal_pos[1] - p_curr[1])**2)
                vmag_command = kmag * distance_error

                print(f'vmag_command: {vmag_command}')
                ...
                if t_sim <= t0:
                    ...
                    t_stabx = t_sim - t0
                    q_joints_des = robot.stabilize_forward(t_stabx, T_stab, z_command, 0.0)
                    robot.p_stable = robot.choose_gait_start()

                elif t_sim > t0 and move_mode!='walk':

                    t_curr = t_sim - (t0 + T_stab)

                    # want to turn first
                    if abs(angle_error) > 1e-1:

                        if abs(angle_error) > 1e-1:
                            if wz_command > 0.0:
                                mode = 'ccw'
                                q_joints_des, all_four = robot.turn_counterclockwise(t_curr, wz_command)
                                t_last = t_curr
                            elif wz_command < -0.1:
                                mode = 'cw'
                                q_joints_des = robot.turn_clockwise(t_curr)
                                t_last = t_curr
                        ...
                        if mode == 'ccw':
                            ...
                            q_joints_des, all_four = robot.turn_clockwise(t_curr, wz_command)
                            t_last = t_curr
                    ...
                    print('entering next stance')

                    # reset the bot position
                    t_reset = t_sim - (t0 + T_stab + t_last)

                    if t_reset < T_stab:
                        q_joints_des = robot.reset(t_reset, T_stab)

                    elif t_sim > (t0 + T_stab + t_last + T_stab) and t_sim < (t0 + T_stab + t_last + T_stab + T_stab):
                        t_stable = t_sim - (t0 + T_stab + t_last + T_stab)
                        q_joints_des = robot.stabilize_forward(t_stable, T_stab, z_command, 0.2)
                        _ = robot.choose_gait_start()

                    else:
                        t_walk = t_sim - (t0 + T_stab + t_last + T_stab + T_stab)
                        q_joints_des = robot.walk_forward(t_walk, vmag_command)
...
# TURN IN THETA
# CLOSE THE Y GAP
# DO THIS SEQUENTIALLY TO AVOID THE NEED FOR BLENDING GAITS