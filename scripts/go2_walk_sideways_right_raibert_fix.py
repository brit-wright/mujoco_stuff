...
# Problems
# Some weird arcing happens during the sideways walking 
# --> I've managed to minimize it for the most part by keeping the stabilization function
# Works better at higher gains, higher foot reaches, and higher body heights
...
class Trajectory():
    ...
    def __init__(self):
        ...
        # self.single_stance_time = 7 * self.cycle_len/16
        # self.double_stance_time = self.cycle_len/16

        self.single_stance_time = 15 * self.cycle_len/32
        self.double_stance_time = self.cycle_len/32
    ...
    def stabilize(self, t, T, z_comm, vy): # assume deltax is zero
        ...
        pf_FLy = self.p0_thigh_fl[1] - self.delty/4


        p0_FRy = self.p0_foot_fr[1]
        pf_FRy = self.p0_thigh_fr[1] + self.delty/4

        p0_RLy = self.p0_foot_rl[1]
        pf_RLy = self.p0_thigh_rl[1] + self.delty/4

        p0_RRy = self.p0_foot_rr[1]
        pf_RRy = self.p0_thigh_rr[1] - self.delty/4
    ...
    def evaluate(self, t, dt):
        ...
        v_base_x_curr = data.qvel[mj_idx.VEL_X]
        # print(f'current y-velocity: {v_base_y_curr}')
        print(f'current x-position: {data.qpos[mj_idx.POS_X]}')
...
if __name__ == '__main__':
    ...
    while (not glfw.window_should_close(window)) and (t_sim < max_sim_time):
        ...
        while (t_sim - t0_sim) < (dt_render):
            ...
            # TODO 1: Fix the body lowering thing --> why does the z-height drift downwards?
            # TODO 2: Play with the double stance period (make longer)
            # TODO 3: For z_curr definition change from parabola to cubic spline --> to get better velocity control -- UPDATE change to Bezier curves
            # TODO 4: Read notes on Bezier curves

            # TODO 1: Completed!
            # 1: To fix the z-axis shifting, chat recommends defining the z-values with a quadratic Bezier curve instead of a parabola
            # 2. A quick temporary fix is to just reset the z-values for the feet every few cycles
            # The quick fix works. No progressive squatting. Also less turning during the walking cycles
