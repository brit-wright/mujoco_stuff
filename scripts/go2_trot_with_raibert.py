...
class Trajectory():
    ...
    def evaluate(self, t, dt):
        ...
        # define the x positions of the legs based on information we have about the body's x velocity

        v_base_x_curr = data.qvel[mj_idx.VEL_X]

        # get the desired v_base_x
        v_base_x_desired = 0.0

        theta0_FL = 0.0

        kv = 0.01
        # kv = 0.04 # falls over if i go to 0.05
        ...
        print(f'X-velocity of the base is: {data.qvel[mj_idx.VEL_X]} m/s')
        print(f'current error is: {v_base_x_curr-v_base_x_desired}')
