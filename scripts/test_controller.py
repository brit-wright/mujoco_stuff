...
from QuadrupedControllerv0 import QuadrupedController
        ...
        while (t_sim - t0_sim) < (dt_render):
            ...
            if counter % decimation == 0:
                ...
                if t_sim <= t0:
                    ...
                    q_joints_des = robot.walk_forward(t_curr, dt_sim)

                elif t_sim > (t0 + T_stab + T_run) and (t_sim - t0 - T_stab - T_run) <= T_reset:

                    t_curr = t_sim - (t0 + T_stab + T_run)

                    q_joints_des = robot.reset(t_curr, T_reset)

                elif t_sim > (t0 + T_stab + T_run + T_reset) and (t_sim - t0 - T_stab - T_run - T_reset) <= T_stab:

                    t_curr = t_sim - (t0 + T_stab + T_run + T_reset)

                    q_joints_des = robot.stabilize_forward(t_curr, T_stab, z_command, v_bod)

                else:
                    
                    t_curr = t_sim - (t0 + T_stab + T_run + T_reset + T_stab)
