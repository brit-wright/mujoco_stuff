...
class QuadrupedController:
    ...
    def walk_forward(self, t, vmag_command, yaw_error):
        ...
        v_base_y_curr = (rot_mat @ self.data.qvel[self.mj_idx.v_base_vel_idx])[1]
    ...
    def walker(self, t, w_command, v_command, angle_error, distance_error):
        ...
        if abs(angle_error) > 0.7 and (self.mode == 'forward' or self.mode == 'walking_fin_recover'):
            ...
            if abs(angle_error) > 1e-1 and self.mode=='':
                ...
                self.mode = 'forward'
            elif abs(distance_error) < 1e-1 and self.all_four_walk == False:
                self.mode = 'walking_fin'
            elif abs(distance_error) < 1e-1 and self.all_four_walk == True:
                self.mode = 'reset'
        ...
        if self.mode == 'turning':
            ...
            # next enter front walk stabilization mode
                ...
                self.q_joints = self.stabilize_forward(t_curr, self.T_stab, self.zcomm, 0.6)
            ...
            # next enter walk mode
                ...
                self.q_joints = self.walk_forward(t_curr, 0.6, angle_error)
                self.t_walk_fin = t
