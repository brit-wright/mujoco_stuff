    ...
    while (not glfw.window_should_close(window)) and (t_sim < max_sim_time):
        ...
        while (t_sim - t0_sim) < (dt_render):
            ...
            if counter % decimation == 0:
                ...
                # stretch right left forward

                # # pitch right thigh up
                # q_joints_des[4] = -fact

                # if q_joints_des[4] <= -1.53:
                #     q_joints_des[4] = -1.53

                # # curl the right leg in
                # q_joints_des[4] = fact*0.8
                # q_joints_des[5] = -fact*4

                # if q_joints_des[4] >= 1.53 and q_joints_des[5] <= -2.7:
                #     q_joints_des[4] = 1.53
                #     q_joints_des[5] = -2.7
