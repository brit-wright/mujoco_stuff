...
class QuadrupedController:
    ...
    def walk_forward(self, t, vmag_command):
    ...
    def turn_counterclockwise(self, t, wz_command):
        ...
        return self.qcurr, all_four_down
    ...
    #     pdes_x_FL = (1 - alpha) * self.pdlast[0] + alpha * self.pdlast[0]
    #     pdes_y_FL = (1 - alpha) * self.pdlast[1] + alpha * self.pdlast[1]
    #     pdes_z_FL = (1 - alpha) * self.pdlast[2] + alpha * -0.35
    #     pcurr_FL = [pdes_x_FL, pdes_y_FL, pdes_z_FL]

    #     pdes_x_FR = (1 - alpha) * self.pdlast[3] + alpha * self.pdlast[3]
    #     pdes_y_FR = (1 - alpha) * self.pdlast[4] + alpha * self.pdlast[4]
    #     pdes_z_FR = (1 - alpha) * self.pdlast[5] + alpha * -0.35
    #     pcurr_FR = [pdes_x_FR, pdes_y_FR, pdes_z_FR]

    #     pdes_x_RL = (1 - alpha) * self.pdlast[6] + alpha * self.pdlast[6]
    #     pdes_y_RL = (1 - alpha) * self.pdlast[7] + alpha * self.pdlast[7]
    #     pdes_z_RL = (1 - alpha) * self.pdlast[8] + alpha * -0.35
    #     pcurr_RL = [pdes_x_RL, pdes_y_RL, pdes_z_RL]

    #     pdes_x_RR = (1 - alpha) * self.pdlast[9] + alpha * self.pdlast[9]
    #     pdes_y_RR = (1 - alpha) * self.pdlast[10] + alpha * self.pdlast[10]
    #     pdes_z_RR = (1 - alpha) * self.pdlast[11] + alpha * -0.35
    #     pcurr_RR = [pdes_x_RR, pdes_y_RR, pdes_z_RR]

    #     # calculate the inverse kinematics
    #     theta_FL = NewtonRaphson(self.pinit[0:3], pcurr_FL, self.qcurr[0:3], self.chain_base_foot_fl, self.chain_base_hip_fl).call_newton_raphson()
    #     theta_FR = NewtonRaphson(self.pinit[3:6], pcurr_FR, self.qcurr[3:6], self.chain_base_foot_fr, self.chain_base_hip_fr).call_newton_raphson()
    #     theta_RL = NewtonRaphson(self.pinit[6:9], pcurr_RL, self.qcurr[6:9], self.chain_base_foot_rl, self.chain_base_hip_rl).call_newton_raphson()
    #     theta_RR = NewtonRaphson(self.pinit[9:12], pcurr_RR, self.qcurr[9:12], self.chain_base_foot_rr, self.chain_base_hip_rr).call_newton_raphson()
    ...
    #     self.pinit = np.concatenate((pcurr_FL, pcurr_FR, pcurr_RL, pcurr_RR))
    #     return self.qreset
