...
class Trajectory():
    ...
    def evaluate(self, t, dt, z_command):

        pd_leg = np.concatenate((self.p_interest1, self.p_interest2, self.p_interest3, self.p_interest4))
        zd_body = np.array([0.0, 0.0, z_command])

        print(f'the z_command is: {z_command}')
        ...
        r_zd_toe = toe_pos
        r_zd_toe[2] = -z_command
        r_zd_thigh = thigh_pos
        
        r_base_FL = r_zd_toe - r_zd_thigh # the vector pointing from the thigh to the toe
        ...
        r_base_FR = r_zd_toe - r_zd_thigh # the vector pointing from the thigh to the toe
        ...
        r_base_RL = r_zd_toe - r_zd_thigh # the vector pointing from the thigh to the toe
        ...
        r_base_RR = r_zd_toe - r_zd_thigh # the vector pointing from the thigh to the toe
    ...
    while (not glfw.window_should_close(window)) and (t_sim < max_sim_time):
        ...
        while (t_sim - t0_sim) < (dt_render):
            ...
            if counter % decimation == 0:
                ...
                if t_sim == 0.0:
                    q_joints_des = np.zeros_like(q_joints) # this is the initial config from the yaml file

                    # get the initial z_height
                    z_base_init = data.qpos[mj_idx.POS_Z]
                    print(f'INITIAL Z HEIGHT: {z_base_init}')
                    ...
                    if z_comm >= 0.21 and res == False:
                        z_comm -= 0.0001
                
                    q_joints_des = traj.evaluate(t0_sim, dt_sim, z_comm)
