...
class QuadrupedController:
    ...
    def walk_forward(self, t, vmag_command, yaw_curr):
        ...
        # CREATING AN ACTUAL LIST OF GOOD CONSTANTS
        ...
        # if vmag_command == 0.8:
        #     kv = -0.06
        # elif vmag_command < 0.8 and vmag_command > 0.6:
        #     kv = -0.015
        # elif vmag_command <= 0.6:
        #     kv = 0.00015

        # kv = -0.06

        kv = -0.06 * vmag_command/0.8
    ...
    def walker(self, t, w_command, v_command, angle_error, distance_error, angle_curr):
        ...
        if abs(angle_error) > 0.7 and abs(distance_error) > 0.2 and (self.mode == 'forward' or self.mode == 'walking_fin_recover'):
            ...
            if abs(angle_error) > 1e-1 and self.mode=='':
                ...
                self.mode = 'forward'
            elif abs(distance_error) < 0.9e-1 and self.all_four_walk == False:
                self.mode = 'walking_fin'
            elif abs(distance_error) < 0.9e-1 and self.all_four_walk == True:
                self.mode = 'reset'
